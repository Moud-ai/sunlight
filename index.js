...
// NOTE: no static `import` of ./App on purpose. Babel hoists ES imports above
// every other statement, which evaluates the entire screen graph before the
// error handler below exists. CommonJS require() runs in written order, so
// the handler (and boot journal) install first.
...
AppRegistry.registerComponent(appName, () => {
  try {
    ...
    // everything and leave a blank screen with no trace.
    if (bootMark) {
