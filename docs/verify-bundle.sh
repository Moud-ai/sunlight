...
#
# 1. Structural integrity: hermesc -b -dump-bytecode on the HBC file.
# 2. Execution smoke test: run the bundle through a real Hermes VM built from
#    the source tag that produced libhermesvm.so (see docs/GRAYSCREEN_CAUSE_MAP).
#    The VM stops at RN core init (missing native globals), which is expected —
#    the point is to prove the bundle evaluates WITHOUT app-level JS errors.
#
...
for m in run-start 'last run failed' '@sunlight_boot_log' '@sunlight_last_error'; do
