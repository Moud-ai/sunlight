 ...
 * un-debuggable gray screens into actionable text without adb.
 *
    ...
    AsyncStorage.getItem(KEY)
      .then(raw => {
        ...
        if (raw == null) {
          ...
          journal = safelyParse(raw);
        }
        if (stage === 'run-start') {
          // Archive the previous run so a later launch can diagnose it.
          journal.prev =
...
/** Read the full journal (previous + current run). Null when unavailable. */
export async function readBootJournal(): Promise<Journal | null> {
  try {
...
export async function readPreviousFailure(): Promise<string | null> {
  const journal = await readBootJournal();
  if (journal == null || journal.prev.length === 0) {
    return null;
  }
